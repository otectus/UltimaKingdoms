#!/usr/bin/env python3
"""R4 private history and opposing outcomes through two real clients."""
...
def main():
    ...
    time.sleep(1)
    try:
        slog=server/'console.log';srv=spawn(['java','-Xmx2G','-Dultima.acceptance.r4Multiplayer=true','@libraries/net/minecraftforge/forge/1.20.1-47.4.23/unix_args.txt','nogui'],server,slog)
            ...
            def expand(arguments):
                ...
                for arg in arguments:
                    ...
                    for value in arg if isinstance(arg,list) else [arg]:
                        ...
                        result.append(value)
                return result
            command=['java','-Xmx2G',f'-Dultima.clientTest.output={folder}','-Dultima.clientTest.evolutionMultiplayer=true',f'-Dultima.clientTest.shared={work}',f'-Dultima.clientTest.role={role}',f'-Dultima.clientTest.server=127.0.0.1:{port}']+expand(vanilla['arguments']['jvm']+forge['arguments']['jvm'])+[forge['mainClass']]+expand(vanilla['arguments']['game']+forge['arguments']['game'])+['--width','960','--height','640']
        ...
        srv.stdin.write('r4-fixture setup\n');srv.stdin.flush()
        wait_until(lambda:'R4_MULTIPLAYER_READY' in slog.read_text(errors='replace'))
        wait_until(lambda:all((work/(role+'-READ')).exists() for role in ['leader','stranger']))
        (work/'RACE').write_text('send opposing choices')
        wait_until(lambda:all((work/(role+'-SENT')).exists() for role in ['leader','stranger']))
        time.sleep(2)
        srv.stdin.write('r4-fixture audit\n');srv.stdin.flush()
        wait_until(lambda:'R4_MULTIPLAYER_AUDIT_PASS' in slog.read_text(errors='replace'))
