...
public final class McaQuestsApi {
    ...
    /**
     * Publishes a complete, already viewer-filtered atlas snapshot. The deliberately map-shaped input
     * keeps optional integrations reflection-only; MCAQuests validates every field before networking it.
     */
    public static void publishExternalMapPoints(ServerPlayer player,String owner,List<?> encoded){
        if(player==null||owner==null||encoded==null)throw new NullPointerException("external map snapshot");
        if(player.getServer()==null||!player.getServer().isSameThread())throw new IllegalStateException("external map publication requires server thread");
        if(!owner.matches("[a-z0-9_.-]{1,64}")||encoded.size()>128)throw new IllegalArgumentException("invalid external map snapshot");
        List<ExternalMapPoint> points=new ArrayList<>(encoded.size());for(Object raw:encoded){
            if(!(raw instanceof Map<?,?> value))throw new IllegalArgumentException("map point must be a map");
            String key=string(value,"key",160),dimension=string(value,"dimension",128),label=string(value,"label",128),kind=string(value,"kind",32);
            int x=number(value,"x"),y=number(value,"y"),z=number(value,"z");boolean approximate=bool(value,"approximate"),lastKnown=bool(value,"lastKnown");
            points.add(new ExternalMapPoint(key,ExternalMapPoint.dimension(dimension),new BlockPos(x,y,z),label,
                    "route".equals(kind)?ExternalMapPoint.Kind.ROUTE:ExternalMapPoint.Kind.SITE,approximate,lastKnown));
        }
        QuestNetwork.CHANNEL.send(PacketDistributor.PLAYER.with(()->player),new ExternalMapPointsS2CPacket(owner,points));
    }
    private static String string(Map<?,?> map,String key,int max){Object value=map.get(key);if(!(value instanceof String text)||text.length()>max)return fail(key);return text;}
    private static int number(Map<?,?> map,String key){Object value=map.get(key);if(!(value instanceof Number number))throw new IllegalArgumentException("invalid "+key);return number.intValue();}
    private static boolean bool(Map<?,?> map,String key){Object value=map.get(key);if(!(value instanceof Boolean flag))throw new IllegalArgumentException("invalid "+key);return flag;}
    private static <T> T fail(String key){throw new IllegalArgumentException("invalid "+key);}
